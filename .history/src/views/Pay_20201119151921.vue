  ...
  <div class="pay">
    ...
    <div class="addressbox">
      <div class="address-text" @click="showSelectAddressList">
        <div class="title">{{currentAddress.address}} <van-icon name="arrow" /></div>
        <div class="userinfo">{{currentAddress.name}} {{currentAddress.tel}}</div>
      </div>
    </div>
    <div class="productlist">
      <div class="product-box">
        <div class="pro-title">商品列表</div>
        <div class="product-item" v-for="(item,index) in productsData" :key="index">
    ...
    <div class="gopay">
      <van-submit-bar label=" " :price="productInfo.total*100" button-text="去支付" />
    </div>
    <van-popup is-link position="bottom" closeable v-model="isOpen">
      ...
      <van-address-list
        ...
        @edit="this.$router.push({name:'AddressList'})"
      />
      </van-popup>
  </div>
</template>
